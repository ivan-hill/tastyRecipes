...
describe('Implement the screen  Food Recipe Details', () => {
  test('1- check the path to the details page', async () => {
    fetch(urlAll);
  ...
  test('2- verify if the elements are on the detail page', async () => {
    fetch(urlAll);
    ...
    userEvent.click(recipeCorba);
    expect(history.location.pathname).toBe('/foods/52977');
    const recipeCorbaTitle = await screen.findByRole('heading', {
      name: /corba/i,
    });
    expect(recipeCorbaTitle).toBeInTheDocument();

    const favoriteRecipeButton = screen.getByRole('button', {
      name: /favorite/i,
    });
    ...
    const startRecipeButton = screen.getByRole('button', {
      name: /start recipe/i,
    });
    ...
    const verifyLocalStorageRecipeInProgress = JSON.parse(
  ...
  test('3- check details page and recipe completion', async () => {
    fetch(urlAll);
    ...
    const burekRecipeDetails = await screen.findByRole('heading', {
      name: /burek/i,
    });
    expect(burekRecipeDetails).toBeInTheDocument();

    const favoriteRecipeButton2 = screen.getByRole('button', {
      name: /favorite/i,
    });
    ...
    expect(favoriteRecipeButton2).toHaveAttribute('src', BLACK_HEART);
    const burekIngridient1 = await screen.findByRole('checkbox', {
    ...
    const burekIngridient6 = await screen.findByRole('checkbox', {
      name: /pepper - dash/i,
    });
    ...
    const doneRecipeTitle = await screen.findByRole('heading', {
      name: /done recipes/i,
    });
    ...
    const doneRecipesLocalStorage = JSON.parse(
    ...
    const profileIcon = screen.getByRole('button', {
      name: /desenho de uma silhueta humana/i,
    });
