...
function CategoriesButtons() {
  ...
  // status of the foods page categoriesestado das categorias da pagina foods
  const [categories, setCategories] = useState([]); // button array to be rendered according to API call

  const [isClicked, setIsClicked] = useState('');
  const { pathname } = useLocation();

  // checks which page you are on and changes endPoints
  if (pathname === '/foods') {
  ...
  // API call according to the page
  ...
  const choosenCategoryOnClick = ({ target: value }) => {
    ...
    if (pathname === '/foods' && isClicked === name) {
      setCallApi('https://www.themealdb.com/api/json/v1/1/search.php?s='); // setCall endpoint
    } else {
